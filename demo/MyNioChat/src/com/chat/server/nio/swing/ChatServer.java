package com.chat.server.nio.swing;
...
public class ChatServer implements Runnable {
	...
	private void readMsg(SelectionKey key) throws IOException {
		...
		// 客户端传过来的数据格式是"open_aaa"，表示名称为aaa的用户请求打开聊天窗体
		...
		if (str.indexOf("open_") != -1) {// 客户端连接服务器
			...
			// 获取选择器上的已选择的key并迭代
			// 将更新后的名称列表数据附加到每个套接字通道key上,并重设key感兴趣---------------`的操作
			Iterator<SelectionKey> iter = key.selector().selectedKeys()
					.iterator();
			while (iter.hasNext()) {
				SelectionKey selKey = iter.next();
