...
import com.chat.server.netty.EchoClient;
...
public class ChatFrame {
	...
	private EchoClient service;// 用于与服务器交互

	public ChatFrame(EchoClient service, String uname) {
		this.uname = uname;
	...
	private void init() {
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (msg.length() > 0) {
					try {
						service.send(uname + "^" + writeContext.getText());
					} catch (Exception e1) {
						e1.printStackTrace();
					}
				}
