...
public class ChatFrame {
	...
	private SecureChatClient service ;
	private Channel ch;
	private String uname;// 用户姓名

	public ChatFrame(String uname, SecureChatClient service) {
		this.uname = uname;
...
//					service.send("exit_" + uname);
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (msg.length() > 0) {
					try {
						System.out.println("持久时候的通道"+ch.getClass());
						System.out.println("持久时候的group是否关闭"+ch.eventLoop().parent().isShutdown());
						ch.pipeline().writeAndFlush(msg);
//						ch.writeAndFlush(msg);
						System.out.println("writeAndFlush  over");
					} catch (Exception e1) {
						e1.printStackTrace();
					}
				}
	...
	public void show() {
		this.init();
		try {
			ch=service.init(uname);
			System.out.println("通道是否保持长连接："+ch.isActive());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
